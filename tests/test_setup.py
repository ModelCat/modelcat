...
from modelcat.modelcatconnector.setup import run_setup, setup_cli


class TestSetup(unittest.TestCase):
    """Test cases for the setup module."""

    @patch('modelcat.modelcatconnector.setup.check_awscli')
    @patch('modelcat.modelcatconnector.setup.input')
    @patch('modelcat.modelcatconnector.setup.getpass')
    @patch('modelcat.modelcatconnector.setup.ProductAPIClient')
    @patch('modelcat.modelcatconnector.setup.run_cli_command')
    @patch('modelcat.modelcatconnector.setup.check_aws_configuration')
    @patch('modelcat.modelcatconnector.utils.aws.check_s3_access')
    @patch('modelcat.modelcatconnector.setup.os.makedirs')
    ...
    @patch('modelcat.modelcatconnector.setup.run_setup')
    ...
    def test_run_setup_api_error(self, mock_api_client, mock_getpass, mock_input, mock_check_awscli):
        ...
        from modelcat.modelcatconnector.utils.api import APIError
        mock_client_instance = MagicMock()
